    ...
    <article className={styles.card}>
      ...
      <div className={styles.mediaWrapper}>
        ...
        {post.mediaUrl?.length > 1 && (
          <>
            {mediaIndex > 0 && (
              <button className={`${styles.carouselBtn} ${styles.carouselPrev}`} onClick={() => setMediaIndex(i => i - 1)}>‹</button>
            )}
            {mediaIndex < post.mediaUrl.length - 1 && (
              <button className={`${styles.carouselBtn} ${styles.carouselNext}`} onClick={() => setMediaIndex(i => i + 1)}>›</button>
            )}
