...
import { getUserProfile, getUserPosts, getUserReels, toggleFollow, deletePost, deleteReel } from '../utils/api';
...
export default function ProfilePage() {
  ...
  const handleDelete = async (e, item) => {
    e.preventDefault();
    e.stopPropagation();
    if (!window.confirm(`Delete this ${tab === 'posts' ? 'post' : 'reel'}?`)) return;
    try {
      if (tab === 'posts') {
